...
from rome_files.rome.tok_dataset import (
    TokenizedDataset,
    dict_to_,
...
from rome_files.util.runningstats import Covariance, tally
...
def predict_from_input(model, inp):    # origin
    out = model(**inp)["logits"]
    probs = torch.softmax(out[:, -1], dim=1)   # compute the probs
    p, preds = torch.max(probs, dim=1)         # take the maximums
    return preds, p
